motion_feature_preparation <-function(patient_clinical_data,seg_window,mf_choice,sr_choice,outerFolds,impNo = 1,saveDir,DSseed = 2020){
  ...
  for (l in 1:length(outerFolds)){
   ...
   for (j in 1:length(curr_outerFold)){
     ...
     train_DF <- cbind(as.data.frame(curr_LOL$Xr[,1:r]),down_train[,c(apacheIIriskIdx,labelIdx)])
     names(train_DF)<-c(paste0(rep("MF.",r),1:r),"APACHEIIMortalityRisk",curr_label)
     test_DF <- cbind(as.data.frame(as.matrix(curr_test[,mf_col_idx])%*%curr_LOL$A[,1:r]),curr_test[,c(apacheIIriskIdx,labelIdx)])
     names(test_DF)<-c(paste0(rep("MF.",r),1:r),"APACHEIIMortalityRisk",curr_label)
