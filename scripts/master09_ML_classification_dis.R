...
for (i in 1:length(cvIdx)) {
  ...
  tod_GOSE_self_val <-
    predict_SVM(tod_GOSE_SVM, train_tod_GOSE_CV, Y, currTrainIdx, TRUE)
  tfr_GOSE_self_val <-
    predict_SVM(tfr_GOSE_SVM, train_tfr_GOSE_CV, Y, currTrainIdx, TRUE)
  
  tod_fav_self_val <-
    predict_SVM(tod_fav_SVM, train_tod_fav_CV, Y, currTrainIdx, TRUE)
  tfr_fav_self_val <-
    predict_SVM(tfr_fav_SVM, train_tfr_fav_CV, Y, currTrainIdx, TRUE)
  
  Y <- as.factor(patient_clinical_data$death)
  
  tod_death_self_val <-
    predict_SVM(tod_death_SVM, train_tod_death_CV, Y, currTrainIdx, TRUE)
  tfr_death_self_val <-
    predict_SVM(tfr_death_SVM, train_tfr_death_CV, Y, currTrainIdx, TRUE)
  ...
  tod_GOSE_test_val <-
    predict_SVM(tod_GOSE_SVM, test_tod_GOSE_CV, Y, currTestIdx, FALSE)
  tfr_GOSE_test_val <-
    predict_SVM(tfr_GOSE_SVM, test_tfr_GOSE_CV, Y, currTestIdx, FALSE)
  
  tod_fav_test_val <-
    predict_SVM(tod_fav_SVM, test_tod_fav_CV, Y, currTestIdx, FALSE)
  tfr_fav_test_val <-
    predict_SVM(tfr_fav_SVM, test_tfr_fav_CV, Y, currTestIdx, FALSE)
  
  Y <- as.factor(patient_clinical_data$death)
  
  tod_death_test_val <-
    predict_SVM(tod_death_SVM, test_tod_death_CV, Y, currTestIdx, FALSE)
  tfr_death_test_val <-
    predict_SVM(tfr_death_SVM, test_tfr_death_CV, Y, currTestIdx, FALSE)
