...
return (lapply(c(2:14), function(y) {
          lapply(seq_along(list_of_preds[[1]]), function(x) {
            evalmod(scores=list(list_of_preds[[1]][[x]][[y]],
                       list_of_preds[[2]][[x]][[y]],
                       list_of_preds[[3]][[x]][[y]],
                       list_of_preds[[4]][[x]][[y]],
                       list_of_preds[[5]][[x]][[y]]),
                  labels=(list(list_of_preds[[1]][[x]]$ground_truth,
                       list_of_preds[[2]][[x]]$ground_truth,
                       list_of_preds[[3]][[x]]$ground_truth,
                       list_of_preds[[4]][[x]]$ground_truth,
                       list_of_preds[[5]][[x]]$ground_truth)),dsids=c(1:5))})
          }))
}
...
get_precrec_plots <- function(plot_info, auc_info){
  ...
  val<-names(auc_info[[1]])
  lapply(c(1:6), function(y) {
    png(paste0(filePath,val[y],"_precrec.png"))
      lapply(seq_along(plot_info), function(x) {
      plot(plot_info[[x]][[y]], "PRC",col=color_choice[x], lwd=2) 
      par(new=TRUE) 
      })
    legend("bottomright", legend=names(color_choice),fill=color_choice,cex=0.75)  
    title(val[y])
...
get_auc_mean_sd <- function(plot_info, auc_info){
  ...
  lapply(c(1:6), function(y) {
      ...
      info_list[[y]]<<-data.frame(mean=means,stddev=stddevs, row.names=gsub("_X1","",names(auc_info[[1]][[1]])[2:14]))
  })
